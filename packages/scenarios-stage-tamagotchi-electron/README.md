...
This package owns product-specific Electron scenario definitions only. It depends on `@proj-nova/vishot-runner-electron` for:
...
1. Build `@proj-nova/stage-tamagotchi`.
2. Run this scenario through `@proj-nova/vishot-runner-electron`.
...
When running scenarios through `@proj-nova/vishot-runner-electron`, the built Electron app can use a different `userData` profile than `dev:tamagotchi`.
