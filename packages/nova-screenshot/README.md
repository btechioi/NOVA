...
```bash
pnpm -F @proj-nova/airi-screenshot capture tamagotchi --scenario settings-connection --output-dir .vishot/pr-123
```

Use an explicit scenario path when no preset exists:

```bash
pnpm -F @proj-nova/airi-screenshot capture tamagotchi --scenario packages/scenarios-stage-tamagotchi-electron/src/scenarios/demo-hearing-dialog.ts --output-dir .vishot/hearing --format avif
```
...
underlying `@proj-nova/vishot-runner-*` packages directly when authoring or
