...
const androidVersionCodePattern = /^NOVA_VERSION_CODE=(\d+)$/m
...
async function syncAndroidVersion(version: string) {
  ...
  if (!currentVersionCodeMatch) {
    throw new TypeError('Android version file does not contain a valid NOVA_VERSION_CODE')
  }
  ...
  if (!Number.isSafeInteger(currentVersionCode) || currentVersionCode < 1) {
    throw new TypeError(`Android NOVA_VERSION_CODE is invalid: ${currentVersionCodeMatch[1]}`)
  }

  const nextVersionCode = currentVersionCode + 1
  const nextAndroidVersionContent = `NOVA_VERSION_NAME=${version}\nNOVA_VERSION_CODE=${nextVersionCode}\n`
